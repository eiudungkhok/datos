...
<div className="qr-box"><img src={d.qr_image_url} alt="QR" className="cursor-pointer" onClick={() => setSelectedImage(d.qr_image_url)} /></div>                          <div className="acc-info mt-2 text-center"><p>{d.account_name}</p><h4 className="mt-2" style={{color: d.color_theme}}>{d.account_number}</h4></div>
          ...
          {activeSection === "projects" && (
            <div className="section active"><h2 className="section-title glitch-text" data-text="DATABASE_OF_PROJECTS">DATABASE_OF_PROJECTS</h2><div className="project-grid mt-2">{projectsList.map((p) => (<div key={p.id} className="project-card holographic-card"><div className="project-img-wrapper"><img src={p.image_url} alt={p.title} className="cursor-pointer" onClick={() => setSelectedImage(p.image_url)} /></div></div><div className="project-info"><h3 style={{color: "var(--neon-cyan)"}}>{p.title}</h3><p className="mb-2">{p.description}</p><div style={{ display: "flex", gap: "10px" }}>{p.github && <a href={p.github} target="_blank" className="cyber-btn" style={{padding: "5px 10px", fontSize: "0.8rem"}}><span>[ GITHUB ]</span></a>}{p.demo && p.demo !== '#' && <a href={p.demo} target="_blank" className="cyber-btn secondary" style={{padding: "5px 10px", fontSize: "0.8rem"}}><span>[ DEMO ]</span></a>}</div></div></div>))}</div></div>
          )}
          {activeSection === "media" && (
